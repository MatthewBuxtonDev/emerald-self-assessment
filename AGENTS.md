...
- Mistral AI API (`mistral-small-latest` via OpenAI-compatible endpoint) for AI conversation + report
...
├── lib/          # mistral.ts, prompts.ts, utils.ts
├── hooks/        # React hooks
...
5. **Mistral API key** goes in `.env.local` as `MISTRAL_API_KEY`. Never commit.
...
- Both use `response_format: { type: "json_object" }` for structured output via Mistral
...
- Environment variables: only `MISTRAL_API_KEY` required; `NEXT_PUBLIC_APP_URL` optional
